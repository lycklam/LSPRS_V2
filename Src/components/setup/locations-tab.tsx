...
export default function LocationsTab() {
  ...
  const save = async () => {
    ...
    setSaving(true);
    try {
      const payload = { supplier_id: form.supplier_id, country_id: form.country_id, name: form.name, address: form.address || null, site_type: form.site_type, site_manager: form.site_manager || null, status: "active" as const };
      const { error } = editId
  ...
  const siteLabel = (t: string) => ({ "DC": "Distribution Centre", "cross-dock": "Cross-dock", "last-mile": "Last Mile Hub", "other": "Other" }[t] || t);
    ...
    <div className="space-y-6">
      <div className="bg-white border border-gray-200 rounded-xl p-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="space-y-1"><Label>Location Name *</Label><Input value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} placeholder="e.g. Amsterdam DC" /></div>
          <div className="space-y-1"><Label>Site Type *</Label>
            <Select value={form.site_type} onValueChange={v => setForm({ ...form, site_type: v })}>
              ...
              <SelectContent>
                <SelectItem value="DC">Distribution Centre</SelectItem>
                <SelectItem value="cross-dock">Cross-dock</SelectItem>
                <SelectItem value="last-mile">Last Mile Hub</SelectItem>
                <SelectItem value="other">Other</SelectItem>
              </SelectContent>
            </Select>
          </div>
          <div className="space-y-1"><Label>Address</Label><Input value={form.address} onChange={e => setForm({ ...form, address: e.target.value })} placeholder="Street address" /></div>
          <div className="space-y-1"><Label>Site Manager</Label><Input value={form.site_manager} onChange={e => setForm({ ...form, site_manager: e.target.value })} placeholder="Name" /></div>
        </div>
        <div className="flex gap-3 mt-4">
          <Button onClick={save} disabled={saving || !form.supplier_id || !form.country_id || !form.name || !form.site_type}>{saving ? "Saving..." : editId ? "Update Location" : "+ Add Location"}</Button>
          {editId && <Button variant="outline" onClick={() => { setEditId(null); setForm(EMPTY); }}>Cancel</Button>}
        </div>
      </div>

      <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
        <div className="px-5 py-3 border-b border-gray-100 font-semibold text-sm text-gray-700">Existing Locations ({locations.length})</div>
        {loading ? <div className="p-8 text-center text-gray-400">Loading...</div> : locations.length === 0 ? <div className="p-8 text-center text-gray-400">No locations yet.</div> : (
          <table className="w-full text-sm">
            <thead><tr className="border-b border-gray-100 bg-gray-50">{["Supplier","Country","Location","Site Type","Manager",""].map(h => <th key={h} className="text-left px-5 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">{h}</th>)}</tr></thead>
            <tbody>{locations.map(l => (
              <tr key={l.id} className="border-b border-gray-50 hover:bg-gray-50">
                ...
                <td className="px-5 py-3"><div className="flex gap-2 justify-end"><Button size="sm" variant="outline" onClick={() => startEdit(l)}>Edit</Button><Button size="sm" variant="destructive" onClick={() => setDeleteTarget(l)}>Delete</Button></div></td>
              </tr>
            ))}</tbody>
          </table>
        )}
      ...
      <AlertDialog open={!!deleteTarget} onOpenChange={open => !open && setDeleteTarget(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>This will permanently delete <strong>{deleteTarget?.name}</strong> and ALL linked submissions and responses. This cannot be undone.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
