    ...
    <div style={{ display: 'flex', minHeight: 'calc(100vh - 73px)' }}>
      ...
      <main className="main-content">
        {nav === "review" && (
          <>
            <div className="page-header">
              <div className="page-title">Review & Approve</div>
              <div className="page-subtitle">Review monthly submissions and approve or flag them for follow-up.</div>
            </div>
            <ReviewPanel />
          </>
        )}
        {nav === "ratings" && (
          <>
            <div className="page-header">
              <div className="page-title">Enter Internal Ratings</div>
              <div className="page-subtitle">Rate qualitative Likert metrics for each supplier location.</div>
            </div>
            <EnterRatings />
          </>
        )}
        {nav === "setup" && (
          <div>
            <div className="page-header">
              ...
              <div className="page-subtitle">Manage suppliers, countries, locations and metric relevance settings.</div>
            </div>
            <div className="tab-bar">
              {SETUP_TABS.map(tab => (
                <button
                  key={tab.key}
                  className={`tab-item ${setupTab === tab.key ? 'active' : ''}`}
